    ...
    for (var a = 0; a < printNum; a++) {
      ...
      for (let k = 0; k < 8; k++) {
        ...
        if (((a + 1) * 8 - 8 + k) < data[i]['orderDetails'].length) {
          ...
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '129mm', '20mm', '12mm', data[i]['orderDetails'][(a + 1) * 8 - 8 + k].sellPrice);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '149mm', '20mm', '12mm', data[i]['orderDetails'][(a + 1) * 8 - 8 + k].totalPrice);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
...
const PrintSales3 = (data, codetype, url, code) => {
  function getByteLen(val) {
    var len = 0;
    for (var i = 0; i < val.length; i++) {
      if (val[i].match(/[^\x00-\xff]/ig) != null) {
  ...
  for (let i = 0; i < printArray.length; i++) {
    ...
    for (var a = 0; a < printNum; a++) {
     ...
     /* LODOP.ADD_PRINT_TEXT('22.15mm', '52mm', '26mm', '5.37mm', '产品代码');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);*/
      LODOP.ADD_PRINT_TEXT('22.15mm', '52mm', '40mm', '5.37mm', '产品名称');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      LODOP.ADD_PRINT_TEXT('22.15mm', '92mm', '18mm', '5.37mm', '规格型号');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      LODOP.ADD_PRINT_TEXT('22.15mm', '110mm', '15mm', '5.37mm', '单位');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      LODOP.ADD_PRINT_TEXT('22.15mm', '125mm', '18mm', '5.37mm', '实发数量');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      LODOP.ADD_PRINT_TEXT('22.15mm', '143mm', '11mm', '5.37mm', '单价');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      LODOP.ADD_PRINT_TEXT('22.15mm', '154mm', '15mm', '5.37mm', '金额');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      LODOP.ADD_PRINT_TEXT('22.15mm', '169mm', '31mm', '5.37mm', ' 备注');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      //表格线
      ...
      //添加一条线，参数(开始短点距离头部距离，开始端点距左边距离，结束端点距头部距离，结束端点距左边距离)
      LODOP.ADD_PRINT_LINE('19.31mm', '10mm', '28.11mm', '10mm', 0, 1);  // 序号后竖线  序号
      LODOP.ADD_PRINT_LINE('19.31mm', '52mm', '28.11mm', '52mm', 0, 1);  // 单号后竖线      单号
      LODOP.ADD_PRINT_LINE('19.31mm', '92mm', '28.11mm', '92mm', 0, 1);  // 名称后竖线      名称
      LODOP.ADD_PRINT_LINE('19.31mm', '110mm', '28.11mm', '110mm', 0, 1);// 规格竖线    规格
      LODOP.ADD_PRINT_LINE('19.31mm', '125mm', '28.11mm', '125mm', 0, 1);// 单位后竖线   单位
      LODOP.ADD_PRINT_LINE('19.31mm', '143mm', '28.11mm', '143mm', 0, 1);// 实发数量竖线       实发数量
      LODOP.ADD_PRINT_LINE('19.31mm', '154mm', '28.11mm', '154mm', 0, 1);// 单价竖线       单价
      LODOP.ADD_PRINT_LINE('19.31mm', '169mm', '28.11mm', '169mm', 0, 1);// 金额竖线      金额
      LODOP.ADD_PRINT_LINE('19.31mm', '201.55mm', '28.11mm', '201.55mm', 0, 1);// 后后竖线      备注
      ...
      for (let k = 0; k < 8; k++) {
        ...
        if (((a + 1) * 8 - 8 + k) < printArray[i]['orderDetails'].length) {
         ...
         /* LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '52mm', '27mm', '12mm', printArray[i]['orderDetails'][((a + 1) * 8 - 8 + k)].goodCode);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);*/
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '52mm', '40mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].goodName);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          if (fucCheckLength(printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].standard) > 8) {
            LODOP.ADD_PRINT_TEXT(thHeight + newHeight - 2.5 + 'mm', '92mm', '19mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].standard);
            LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          } else {
            LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '92mm', '19mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].standard);
            LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          }
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '110mm', '15mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].unitOfMea);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '125mm', '18mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].actualNum);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '143mm', '11mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].sellPrice);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '154mm', '15mm', '12mm', printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].totalPrice);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '169mm', '31mm', '12mm', printArray[i].remark);
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          actualNum = Math.round((actualNum + parseInt(printArray[i]['orderDetails'][(a + 1) * 8 - 8 + k].actualNum)) * 100) / 100;
          ...
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '52mm', '40mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '92mm', '19mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '110mm', '15mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '125mm', '18mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '143mm', '11mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '154mm', '15mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '169mm', '31mm', '12mm', '');
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
        }
        ...
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '10mm', trheight + newHeight + 9.37 + 'mm', '10mm', 0, 1);//行号后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '52mm', trheight + newHeight + 9.37 + 'mm', '52mm', 0, 1);//供货商后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '92mm', trheight + newHeight + 9.37 + 'mm', '92mm', 0, 1);//型号后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '110mm', trheight + newHeight + 9.37 + 'mm', '110mm', 0, 1);//型号后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '125mm', trheight + newHeight + 9.37 + 'mm', '125mm', 0, 1);//型号后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '143mm', trheight + newHeight + 9.37 + 'mm', '143mm', 0, 1);//颜色后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '154mm', trheight + newHeight + 9.37 + 'mm', '154mm', 0, 1);//颜色后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '169mm', trheight + newHeight + 9.37 + 'mm', '169mm', 0, 1);//单价后竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight - SizeTmpt * fonsize + 'mm', '201.55mm', trheight + newHeight + 9.37 + 'mm', '201.55mm', 0, 1);//最右竖线
        LODOP.ADD_PRINT_LINE(trheight + newHeight + 9.57 + 'mm', '1mm', trheight + newHeight + 9.57 + 'mm', '201.55mm', 0, 1);//每条数据后加一横线
        if ((trheight + newHeight + 9.57) % fenyeSize <= fenyeSize && (trheight + 9.57 + newHeight) % fenyeSize >= (fenyeSize - 15)) {
...
export {PrintSales, PrintSales2,PrintSales3, PrintReturn}
