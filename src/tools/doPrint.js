...
const PrintReturn = (data, codetype, url, code) => {
  ...
  let fenyeSize = 140,
    num = 2;
  for (let i = 0; i < data.length; i++) {
    if (Math.ceil(data[i].returnOrderDetailVOS.length / 7) > 1) {
      var printNum = Math.ceil(data[i].returnOrderDetailVOS.length / 7)
      for (var a = 0; a < printNum; a++) {
        let bshi;
        let lastheght = 2;
        //设置文本，参数(距离页面头部，距离页面左边距离，文本宽度，文本高度，文本内容
        ...
        LODOP.ADD_PRINT_TEXT('6.11mm', '140.54mm', '60.13mm', '7.37mm', '本单总页数：' + ' 第 '+ ( a + 1 ) + ' 页 总：' + Math.ceil(data[i].returnOrderDetailVOS.length / 7));
        //标题等
        LODOP.SET_PRINT_STYLE('FontSize', 10);
        LODOP.ADD_PRINT_TEXT('22.15mm', '1mm', '11mm', '5.37mm', '序号');
        LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
        ...
        for (let k = 0; k < 7; k++) {
          ...
          LODOP.SET_PRINT_STYLE('FontSize', 10);
          if (((a + 1) * 7 - 7 + k) < data[i]['returnOrderDetailVOS'].length) {
            LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '1mm', '9mm', '9mm', parseInt( 1 + k));
            LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
            LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '11mm', '27mm', '12mm', data[i]['returnOrderDetailVOS'][((a + 1) * 7 - 7 + k)].goodCode);
            LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
            LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '38mm', '40mm', '12mm', data[i]['returnOrderDetailVOS'][(a + 1) * 7 - 7 + k].goodName);
            LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
            if(fucCheckLength(data[i]['returnOrderDetailVOS'][(a + 1) * 7 - 7 + k].standard) > 7) {
              LODOP.ADD_PRINT_TEXT(thHeight + newHeight - 2.5  + 'mm', '78mm', '19mm', '12mm', data[i]['returnOrderDetailVOS'][(a + 1) * 7 - 7 + k].standard);
              LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
            }else{
              LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '78mm', '19mm', '12mm', data[i]['returnOrderDetailVOS'][(a + 1) * 7 - 7 + k].standard);
              LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
            }
            LODOP.ADD_PRINT_TEXT(thHeight + newHeight + 'mm', '96mm', '15mm', '12mm', data[i]['returnOrderDetailVOS'][(a + 1) * 7 - 7 + k].unitOfMea);
            LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      ...
      let fonsize = 4;
      //上面的信息
      console.log(Math.ceil(data[i].returnOrderDetailVOS.length / 7))
      LODOP.SET_PRINT_STYLE('FontSize', 11);
      LODOP.ADD_PRINT_TEXT('12.20mm', '2mm', '60.13mm', '7.37mm', '退货单位：' + data[i].customer);
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 1);
      LODOP.ADD_PRINT_TEXT('13.20mm', '69.54mm', '80.13mm', '7.37mm', '日期：' + data[i].createTime);
      LODOP.ADD_PRINT_TEXT('12.20mm', '140.54mm', '60.13mm', '7.37mm', '单号：' + data[i].returnOrderNum);
      LODOP.ADD_PRINT_TEXT('6.11mm', '140.54mm', '60.13mm', '7.37mm', '本单总页数：' + '第 1 页 总：' + Math.ceil(data[i].returnOrderDetailVOS.length / 7));
      //标题等
      LODOP.SET_PRINT_STYLE('FontSize', 10);
      LODOP.ADD_PRINT_TEXT('22.15mm', '1mm', '11mm', '5.37mm', '序号');
      LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
      ...
      for (let k = 0; k < 7; k++) {
        ...
        if (k < data[i]['returnOrderDetailVOS'].length) {
          ...
          LODOP.SET_PRINT_STYLEA(0, 'Alignment', 2);
          if(fucCheckLength(data[i]['returnOrderDetailVOS'][k].standard) > 7) {
